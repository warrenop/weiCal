...
邮件收到 zip → 解压得到 `.csv` 或 `.xlsx` → 应用右上角「刷新数据」打开**导入向导**。

按交易号自动去重，反复上传同一文件也没事。支付宝的「不计收支」（账户间转移）和未完成交易会自动跳过。

### 导入向导（3 步）

| ① 选来源（带获取指引） | ② 选文件（自动解析预览） | ③ 确认导入（统计 + 去重预估 + 样本） |
|---|---|---|
| ![Wizard step 1](docs/screenshots/15-wizard-step1.png) | ![Wizard step 2](docs/screenshots/16-wizard-step2.png) | ![Wizard step 3](docs/screenshots/17-wizard-step3.png) |

第 3 步在**真正写库前**先展示：识别到的来源、记录数、收支合计、日期区间、**将新增 N 条 / 已存在 M 条**（按交易号预估去重）、主要分类和前几行样本。确认后才提交。
